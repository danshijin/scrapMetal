function mainContent(records){
	...
	for(var i =0; i < records.length; i++){
		...
		html += '<div style="padding:.2em .5em;color:#1968A0;font-weight:bold;" class="crTitle'+records[i].id+'">'+records[i].title+'</div>';
		html += '<div style="padding:.2em .5em;" class="crContext'+records[i].id+'">'+records[i].context+'</div>';
	...
	$(".crDel").each(function(index, element){
		$(element).click(function(){
			var id = $(element).attr("crId");
			CommonUtils.confirm("确认","将删除该企业信息", "delRecord("+id+")");
		});
	});
}
